...
export const seamFinder: TestPattern = {
  ...
  parameters: [
    ...
    { key: 'bandWidth', label: 'Band Width (px)', type: 'number', default: 8, min: 2, max: 32, step: 2 },
  ...
  render(ctx, w, h, params) {
    ...
    const bandW = (params.bandWidth as number) ?? 8;
    ...
    for (let col = 1; col < cols; col++) {
      ...
      const bandCount = Math.floor(h / bandW);
    ...
    for (let row = 1; row < rows; row++) {
      ...
      const bandCount = Math.floor(w / bandW);
      for (let i = 0; i < bandCount; i++) {
        ctx.fillStyle = i % 2 === 0 ? c1 : c2;
